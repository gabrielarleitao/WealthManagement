...
#i have to create 3 scallers (alloc is allocation)
WFC_alloc <- 0.3
SPY_alloc <- 0.25
AGG_alloc <- 0.45

#colnames(joined_returns) <- c("WFC_returns", "SPY_returns", "AGG_returns")
#these have to sum to 1
#those allocations represent the dollar allocation in my portfolio
#these have to sum to 1
...
#create a frame with the monthlyReturn function - the ticker for Russell is VONE
benchmark_returns <- monthlyReturn(getSymbols("VONE", auto.assign = FALSE))
#i wont to add this to my portfolio ths benchmark
...
#how to calculate the standard deviation? sigma
#let's calculate sigma and beta
#let's take a look at the portfolio returns timeseries:
#xxxxxxx(joined_portfolio_ret$portfolio, type=xxxxxx)
#now this is semi-stationary. It has the same mean accross all the series, and different sigma here and there. 

##### Adding a benchmark before we do risk analysis##########
##### We'll use Vanguard Russell 1000 ETF (VONE) ############

#benchmark_returns <- xxxxxxxx(getSymbols(xxxxxxx, auto.assign = FALSE))

#joined_monthlyreturns <- xxxxx(stock1_returns, stock2_returns, fixed_income_returns, xxxxxxx)
...
##############
time_index <- 174 #nrow(joined_monthlyreturns) count of months in data frame
...
#time index - NUMBER - the number is how long i am using the time series to define the 
#standard deviation dataframe$variable
...
#print(c("WFC", xxxxxx, "SPY", xxxxxxx, "AGG", xxxxxxx, "RUS1000", xxxxxxxx))
...
#we run a linear regression with the bencharmk returns (RUS1000) as X and asset returns as Y
summary(WFC_reg)

SPY_reg <- lm(monthly.returns ~ monthly.returns.1 ,data=last_12_months)    #we run a linear regression with the bencharmk returns as X and asset returns as Y
summary(SPY_reg)

AGG_reg <- lm(monthly.returns ~ monthly.returns.2 ,data=last_12_months)    #we run a linear regression with the bencharmk returns as X and asset returns as Y
...
##############
## Calculating TrackingError for last 12 months (need to annualize by sqrt(12))
##############
...
WFC_te <- xxxxxxxxxxxxxxxxx #we need to annualize by sqrt(12)
SPY_te <- xxxxxxxxxxxxxxxx
AGG_te <- xxxxxxxxxxxxxxxx
...
joined_monthlyreturns <- as.data.frame(joined_monthlyreturns) %>%
  mutate(xxxxxxxx = xxxxxxxxxxxxxx)
