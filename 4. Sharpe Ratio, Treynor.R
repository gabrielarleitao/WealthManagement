...
#we run a linear regression with the bencharmk returns as X and asset returns as Y
summary(AGG_reg) 
#take a look at the super low beta, and a p value 
#that is >0.1 - the coefficient is statistically insignificant
...
WFC_treynor <- (mean(joined_monthlyreturns$monthly.returns[(time_index-11) : time_index])-riskfree ) / WFC_reg$coefficients[2] 
#this will give us the beta
...
print(portfolio_treynor)


### Covariance matrix and Correlation ####################
  cov(joined_monthlyreturns, use='complete.obs') 
#this will produce a covariance matrix - correlation 
  cor(joined_monthlyreturns, use='complete.obs') 
  cor(joined_monthlyreturns[(time_index-11) : 
              time_index,], use='complete.obs')
